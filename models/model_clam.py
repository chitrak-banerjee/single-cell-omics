...
class Attn_Net(nn.Module):
	...
	def __init__( self, L=30, D=16, dropout=False, n_classes=1 ): #RL
...
class CLAM_SB(nn.Module):
	def __init__(self, gate = True, size_arg = "small", dropout = False, k_sample=7, n_classes=2,
		instance_loss_fn=nn.CrossEntropyLoss(), subtyping=False):
		super(CLAM_SB, self).__init__()
		#Or self.size_dict = {"small": [1024, 512, 256], "big": [1024, 512, 384]}
		#self.size_dict = { "small": [50, 30, 15], "big": [50, 30, 15] } #RL
		self.size_dict = { "small": [30, 100, 16], "big": [207, 100, 50], "tiny": [30, 100, 16]} #RL
	...
	def inst_eval(self, A, h, classifier): 
		...
		if len(A.shape) == 1:
			A = A.view(1, -1)
			#print('the k_sample value is: ', self.k_sample)
		...
		if instance_eval:
			...
			for i in range(len(self.instance_classifiers)):
				...
				if inst_label == 1: #in-the-class:
					...
					if self.subtyping:
						instance_loss, preds, targets, logits, all_ids = self.inst_eval_out(A, h, classifier)
						all_preds.extend(preds.cpu().numpy())
						all_targets.extend(targets.cpu().numpy())
						#Or
...
class CLAM_MB(CLAM_SB):
	def __init__(self, gate = True, size_arg = "small", dropout = False, k_sample=7, n_classes=2,
		...
		self.size_dict = {"small": [315, 100, 30], "big": [207, 100, 30], "tiny": [30, 100, 16]}
		size = self.size_dict[size_arg]
		fc = [nn.Linear(size[0], size[1]), nn.ReLU()]
		if dropout:
			fc.append(nn.Dropout(0.25))
	...
	def forward(self, h, label=None, instance_eval=False, return_features=False, attention_only=False):
		...
		if instance_eval:
			...
			for i in range(len(self.instance_classifiers)):
				...
				classifier = self.instance_classifiers[i]
				if inst_label == 1: #in-the-class:
					instance_loss, preds, targets, logits, all_ids = self.inst_eval(A[i], h, classifier)
					all_preds.extend(preds.cpu().numpy())
					all_targets.extend(targets.cpu().numpy())
				else: #out-of-the-class
					if self.subtyping:
						instance_loss, preds, targets, logits, all_ids = self.inst_eval_out(A[i], h, classifier)
						all_preds.extend(preds.cpu().numpy())
						all_targets.extend(targets.cpu().numpy())
					else:
						continue
